...
class Category extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['createtime','meta_title','meta_keywords','meta_descr'], 'safe']
        ];
    }
   ...
   /**
    *getPrentId 递归查询所有父类ID
    *
    */
    public function getAncestors($cateid)
    {
	$data = self::find()->where('cateid = :id',[':id' => $cateid])->one();	
	if(empty($data)){
		return [];
	}
	$ancestors = array(
			array('cateid' =>$data['cateid'],'title' => $data['title']),
			);
	if((int)$data['parentid'] != 0){
		$ancestors = array_merge($ancestors,$this->getAncestors($data['parentid']));
	}
	return $ancestors;
